...
public class EmailService {
    ...
    public void sendOrderConfirmationEmail(Order order) {
        try {
            ...
            log.info("Order confirmation email sent successfully to: {} for order: {}", order.getUser().getEmail(),
                    order.getId());
        } catch (Exception e) {
        ...
        try {
            ...
            if (order.getOrderItems() != null && !order.getOrderItems().isEmpty()) {
                for (OrderItem item : order.getOrderItems()) {
                    ...
                    String productImage = item.getProduct().getImageUrl() != null
                    ...
                    String itemSubtotal = String.format("%.2f", item.getPrice()
            ...
            String paymentMethodDisplay = order.getPaymentMethod() != null
                    && "CASH_ON_DELIVERY".equals(order.getPaymentMethod())
                            ? "Cash on Delivery"
                            : "Credit Card (Stripe)";
            String paymentStatusDisplay = order.getPaymentStatus() != null
    ...
    /**
     * Send delivery failed email to customer
     */
    public void sendDeliveryFailedEmail(Order order, Delivery delivery, String failureReason) {
        try {
            if (order == null || order.getUser() == null || order.getUser().getEmail() == null) {
                log.warn("Cannot send delivery failed email: missing order or user data");
                return;
            }
            ...
            String trackingNumber = delivery != null ? delivery.getTrackingNumber() : "N/A";
            String reason = failureReason != null ? failureReason : "Unable to complete delivery";

            String htmlContent = generateDeliveryFailedEmailHtml(
                    customerName,
                    order.getId().toString(),
                    trackingNumber,
                    reason);
            ...
            sendHtmlEmail(
                    ...
                    "Delivery Issue - Order #" + order.getId() + " - Car Parts Store",
                    htmlContent);

            log.info("Delivery failed email sent to: {} for order: {}", order.getUser().getEmail(), order.getId());
        } catch (Exception e) {
            log.error("Failed to send delivery failed email for order {}: {}", order.getId(), e.getMessage(), e);
        }
    }

    /**
     * Generate HTML template for delivery failed email
     */
    private String generateDeliveryFailedEmailHtml(String customerName, String orderId, String trackingNumber,
            String reason) {
        return String.format(
                """
                        <!DOCTYPE html>
                                ...
                                <tr>
                                    <td align="center" style="padding: 40px 0;">
                                        <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                                            <!-- Header -->
                                            <tr>
                                                <td style="padding: 40px 40px 20px 40px; text-align: center; background: linear-gradient(135deg, #ff6b6b 0%%, #ee5a5a 100%%); border-radius: 8px 8px 0 0;">
                                                    <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 600;">
                                                        📦 Delivery Issue
                                                    </h1>
                                                </td>
                                            ...
                                            <tr>
                                                <td style="padding: 40px;">
                                                    ...
                                                    <p style="margin: 0 0 20px 0; color: #666666; font-size: 16px; line-height: 1.6;">
                                                        We're sorry, but there was an issue with the delivery of your order.
                                                    </p>

                                                    <!-- Order Details Box -->
                                                    <div style="background-color: #fff5f5; border-left: 4px solid #ff6b6b; padding: 20px; margin: 30px 0; border-radius: 4px;">
                                                    ...
                                                    <p style="margin: 0 0 20px 0; color: #666666; font-size: 16px; line-height: 1.6;">
                                                        Our team will contact you shortly to arrange a new delivery attempt or discuss alternative options.
                                                    </p>

                                                    <!-- CTA Button -->
                                                    <div style="text-align: center; margin: 30px 0;">
                                                        <a href="%s/profile/orders" style="display: inline-block; padding: 14px 40px; background: linear-gradient(135deg, #667eea 0%%, #764ba2 100%%); color: #ffffff; text-decoration: none; border-radius: 6px; font-size: 16px; font-weight: 600;">
                                                            View My Orders
                                                        </a>
                                                    </div>
                                                    ...
                                                    <p style="margin: 0; color: #999999; font-size: 14px; line-height: 1.6;">
                                                        If you have any questions, please contact our support team at support@carpartsstore.com
                                                    </p>
                                                </td>
                                            </tr>
                        ...
                        """,
                customerName, orderId, trackingNumber, reason, frontendUrl);
    }
}
